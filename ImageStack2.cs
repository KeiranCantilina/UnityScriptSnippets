...
namespace DICOMParser
{
    ...
    /// </summary>
    public class ImageStack2 : MonoBehaviour
    {

        private int[] _data;

        private Texture2D[] _view1Texture2Ds;
        private Texture2D[] _view2Texture2Ds;
        private Texture2D[] _view3Texture2Ds;

        private int[] thirdDims = new int[3];
        ...
        /// </summary>
        public ThreadGroupState StartCreatingTextures(SliceType[] SliceTypes, int[] windowWidths, int[] windowCenters)
        {
            ThreadGroupState state = new ThreadGroupState { TotalProgress = DicomFiles.Length + Width + Height };
            StartCoroutine(CreateTextures(state, SliceTypes, windowWidths, windowCenters));// Modified by KKC                                                  --------------------------------------------------------------------
        ...
        private IEnumerator InitFiles(string folderPath, ThreadGroupState threadGroupState)
        {
...

            {
                ...
                //DebugText.text = "Loading Progress: " + threadGroupState.Progress + " / " + threadGroupState.TotalProgress;

                yield return null;
            }

            Width = DicomFiles[0].GetImageWidth(); // 512 for sample data [KKC]
        ...
        /// <param name="threadGroupState">Thread safe thread-state used to observe progress of one or multiple threads.</param>
        /// <returns>IEnumerator for usage as a coroutine</returns>
        private IEnumerator CreateTextures(ThreadGroupState threadGroupState, SliceType[] SliceTypes, int[] windowWidths, int[] windowCenters)
        {
#if PRINT_USAGE
            Debug.Log(Time.time +
            ...
            _view1Texture2Ds = new Texture2D[DicomFiles.Length];
            _view2Texture2Ds = new Texture2D[Height];
            _view3Texture2Ds = new Texture2D[Width];

            var view1TextureColors = new Color32[DicomFiles.Length][];
            var view2TextureColors = new Color32[Height][];
            var view3TextureColors = new Color32[Width][];

            var view1Progress = new ConcurrentQueue<int>();
            var view2Progress = new ConcurrentQueue<int>();
            var view3Progress = new ConcurrentQueue<int>();

            ConcurrentQueue<int>[] ProgressList = { view1Progress, view2Progress, view3Progress };
            Color32[][][] TextureColors = { view1TextureColors, view2TextureColors, view3TextureColors };

            int[] secondDims = new int[3];
            int[] firstDims = new int[3];
            ...
            for (int i=0; i<3; i++)
            {
                switch (SliceTypes[i])
                {
                    case SliceType.Transversal:
                        StartCreatingTransTextures(threadGroupState, ProgressList[i], _data, DicomFiles, TextureColors[i], windowWidths[i], windowCenters[i], 1);
                        firstDims[i] = Width;
                        secondDims[i] = Height;
                        thirdDims[i] = DicomFiles.Length;
                        break;

                    case SliceType.Sagittal:
                        StartCreatingSagTextures(threadGroupState, ProgressList[i], _data, DicomFiles, TextureColors[i], windowWidths[i], windowCenters[i], 1);
                        firstDims[i] = Height;
                        secondDims[i] = DicomFiles.Length;
                        thirdDims[i] = Width;
                        break;
                     case SliceType.Frontal:
                        StartCreatingFrontTextures(threadGroupState, ProgressList[i], _data, DicomFiles, TextureColors[i], windowWidths[i], windowCenters[i], 1);
                        firstDims[i] = Width;
                        secondDims[i] = DicomFiles.Length;
                        thirdDims[i] = Height;
                        break;
            ...
            while (threadGroupState.Working > 0 || !(ProgressList[0].IsEmpty && ProgressList[1].IsEmpty && ProgressList[2].IsEmpty))
            {
                int current;
                if (ProgressList[0].TryDequeue(out current))
                {

                    CreateTexture2D(firstDims[0], secondDims[0], TextureColors[0], _view1Texture2Ds, current);
                    OnTextureUpdate.Invoke(ViewNumber.View1, current);
                    threadGroupState.IncrementProgress();
                }

                if (ProgressList[1].TryDequeue(out current))
                {
                    CreateTexture2D(firstDims[1], secondDims[1], TextureColors[1], _view2Texture2Ds, current);
                    OnTextureUpdate.Invoke(ViewNumber.View2, current);
                    threadGroupState.IncrementProgress();
                }

                if (ProgressList[2].TryDequeue(out current))
                {
                    CreateTexture2D(firstDims[2], secondDims[2], TextureColors[2], _view3Texture2Ds, current);
                    OnTextureUpdate.Invoke(ViewNumber.View3, current);
                    threadGroupState.IncrementProgress();
                }
                //WriteDebug("Progress: " + threadGroupState.Progress + "/" + threadGroupState.TotalProgress);
                yield return null;
            }
        ...
        /// <returns></returns>
        public Texture2D GetTexture2D(ViewNumber view, int index)
        {
            switch (view)
            {
                case ViewNumber.View1:

                    if (_view1Texture2Ds != null && index < _view1Texture2Ds.Length)
                    {
                        //DebugText.text = "Texture: Showing Transversal texture";
                        return _view1Texture2Ds[index];
                    }
                    else
                    {
                        DebugText.text = "Texture: View 1 texture is null, whoops;";
                        return null;
                    }

                case ViewNumber.View2:
                    if (_view2Texture2Ds != null && index < _view2Texture2Ds.Length)
                    {
                        //DebugText.text = "Texture: Showing Frontal texture";
                        return _view2Texture2Ds[index];
                    }
                    else
                    {
                        DebugText.text = "Texture: View 2 texture is null, whoops;";
                        return null;
                    };
                case ViewNumber.View3:
                    if (_view3Texture2Ds != null && index < _view3Texture2Ds.Length)
                    {
                        //DebugText.text = "Texture: Showing Sagittal texture";
                        return _view3Texture2Ds[index];
                    }
                    else
                    {
                        DebugText.text = "Texture: View 3 texture is null, whoops;";
                        return null;
                    };
                default:
                    DebugText.text = "Texture: FAIL";
        ...
        public bool HasData(ViewNumber view)
        {
            switch (view)
            {
                case ViewNumber.View1:
                    return _view1Texture2Ds != null;
                case ViewNumber.View2:
                    return _view2Texture2Ds != null;
                case ViewNumber.View3:
                    return _view3Texture2Ds != null;
                default:
                    return false;
            }
        }
        ...
        public int GetMaxValue(ViewNumber number)
        {
            switch (number)
            {
                case ViewNumber.View1: return thirdDims[0] - 1;
                case ViewNumber.View2: return thirdDims[1] - 1;
                case ViewNumber.View3: return thirdDims[2] - 1;
                default: return 0;
            }
        }
        ...
        private void StartPreProcessing(ThreadGroupState groupState, IReadOnlyList<DiFile> files, int[] target, int threadCount)
        {
            ...
            for (var i = 0; i < threadCount; ++i)
            {
                ...
                var t = new Thread(() => PreProcess(groupState, files, Width, Height, target, startIndex, endIndex, DebugText))
                {
                    IsBackground = true
                };
                t.Start();
        ...
        private static void PreProcess(ThreadGroupState groupState, IReadOnlyList<DiFile> files, int width, int height,
            IList<int> target, int start, int end, TextMeshPro DebugText)
        {
        ...
        public class TextureUpdate : UnityEvent<ViewNumber, int> { }
